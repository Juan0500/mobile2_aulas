...
class AulaMapasGeolocalizacaoViewModel extends ChangeNotifier {
  ...
  static const LatLng centroInicialPadrao = LatLng(-34.9055557, -56.1852505);
  ...
  Future<void> obterMinhaLocalizacao() async {
    ...
    try {
      ...
      if (!serviceEnabled) {
        _mensagemErro = "Localización desactivada.";
      } else {
        ...
        if (permission == LocationPermission.denied ||
          ...
          _mensagemErro = "Permisos de localización denegados.";
        } else {
      ...
      _mensagemErro = "Error al obtener la ubicación: $e";
    } finally {
      _loading = false;
      notifyListeners();
    }
  ...
  Future<void> buscarRota(LatLng origem, LatLng destino) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        _pontosRota = coords.map((c) => LatLng(c[1], c[0])).toList();
      } else {
        _rotaErro = "Erro na API: ${response.statusCode}";
      }
    } catch (e) {
      _rotaErro = "Erro na requisição: $e";
    } finally {
      _rotaLoading = false;
      notifyListeners();
    }
